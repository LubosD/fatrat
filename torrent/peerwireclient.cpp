...
void PeerWireClient::sendPieceList ( const QBitArray &bitField )
{
	...
	int hpieces = bitField.count (true);
	
	if (hpieces == 0 && pwState & FastExtension)
	{
		const char message[] = {0, 0, 0, 1, 0x0F };
		write ( message, sizeof ( message ) );
	}
	else if(hpieces == bitField.count() && pwState & FastExtension)
	{
		const char message[] = {0, 0, 0, 1, 0x0E };
		write ( message, sizeof ( message ) );
	}
	else
	{
...
void PeerWireClient::processIncomingData()
{
	...
	if ( !receivedHandShake )
	{
		...
		// Read reserved bytes
		QByteArray reserved = read ( 8 );

		if ( reserved[7] & 0x04 ) // FastTrack supported
			pwState |= FastExtension;
	...
	do
	{
		...
		switch ( packet.at ( 0 ) )
		{
			...
			case HaveAll:
			case HaveNone:
				peerPieces.fill(packet.at ( 0 ) == HaveAll);
				emit piecesAvailable ( availablePieces() );
				break;
			case RequestPacket:
			{
