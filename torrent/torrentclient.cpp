...
static int ServerMinPort = 6881;
static int ServerMaxPort = /* 6889 */ 7000;
static const int BlockSize = 16384;
static const int MaxBlocksInProgress = 15;
static const int MaxBlocksInMultiMode = 15;
...
static int MaxUploads = 4;
static const int UploadScheduleInterval = 10000;
static const int EndGamePieces = 15;
...
class TorrentClientPrivate
{
	public:
		...
		QBitArray wantedPieces;
		QMap<QString,bool> wantState;
		int pieceCount;
...
void TorrentClientPrivate::computeWanted()
{
	if(!pieceCount)
		return;
	
	if(metaInfo.fileForm() == MetaInfo::SingleFileForm)
		wantedPieces.fill(true,pieceCount);
	else
	{
		wantedPieces.fill(false,pieceCount);
	
		qint64 pos = 0;
		foreach (const MetaInfoMultiFile &entry, metaInfo.multiFiles())
		{
			bool bDown = true;
			if(wantState.contains(entry.path))
				bDown = wantState[entry.path];
			if(bDown)
			{
				long from = pos / metaInfo.pieceLength();
				long to = (long) ceilf( float(pos+entry.length) / metaInfo.pieceLength() );
				wantedPieces.fill(true, from, to+1);
			}
...
void TorrentClient::setPaused ( bool paused )
{
	...
	else
	{
		...
		// connections.
		qDebug() << "Completed pieces:" << d->completedPieces.count ( true ) << "Piece count:" << d->fileManager.pieceCount();
		d->setState ( (d->completedPieces ^ d->wantedPieces).count ( true ) == d->fileManager.pieceCount()
		              ? Seeding : Searching );
		connectToPeers();
		TorrentServer::instance()->addClient ( this );
	}
...
void TorrentClient::setWantState ( QMap<QString,bool> state )
{
	d->wantState = state;
	d->computeWanted();
	
	if(d->state == TorrentClient::Seeding)
	{
		d->setState ( (d->completedPieces ^ d->wantedPieces).count ( true ) == d->pieceCount ? Seeding : Downloading );
	}
}
...
void TorrentClient::fullVerificationDone()
{
	...
	if(d->completedPieces.size() != d->wantedPieces.size())
		d->wantedPieces.fill ( d->completedPieces.size(), true );
...
void TorrentClient::pieceVerified ( int pieceIndex, bool ok )
{
	...
	if ( !ok )
	{
		cerr << "A piece has failed the hash check\n";
	...
	foreach ( TorrentPeer *peer, d->peers )
	{
		...
		{
			if ( peer->pieces.testBit ( i ) && d->incompletePieces.testBit ( i ) && d->wantedPieces.testBit(i) )
			{
				interesting = true;
				break;
			}
	...
	// call the scheduler.
	int completed = (d->completedPieces ^ d->wantedPieces).count ( true );
	qDebug() << "completed" << completed << "total" << d->pieceCount;
	...
	else
	{
		...
		else if ( (d->incompletePieces & d->wantedPieces).count ( true ) < 15 /*&& d->pendingPieces.size() > d->incompletePieces.count(true)*/ )
			d->setState ( Endgame );
		d->callScheduler();
	}
...
void TorrentClient::connectToPeers()
{
	        ...
	        && ( qrand() % ( ConnectionManager::instance()->maxConnections() / 2 ) ) )
	{
		...
		RateController::instance ( this )->addSocket ( client );
...
void TorrentClient::removeClient()
{
	...
	RateController::instance ( this )->removeSocket ( client );
...
void TorrentClient::schedulePieceForClient ( PeerWireClient *client )
{
	...
	if ( !piece )
	{
		...
		QBitArray incompletePiecesAvailableToClient = (d->incompletePieces & d->wantedPieces);
...
void TorrentClient::requestMore ( PeerWireClient *client )
{
	...
	if ( piecesInProgress.isEmpty() && (d->incompletePieces & d->wantedPieces).count ( true ) )
	{
		d->callScheduler();
		return;
	}
...
qulonglong TorrentClient::done() const
{
	//cout << "count: " << d->pieceCount << "; done: " << d->completedPieces.count(true) << "; len: " << d->metaInfo.pieceLength() << endl;
	return qulonglong ( d->completedPieces.count ( true ) ) * qulonglong ( d->metaInfo.pieceLength() );
}

const char* TorrentClient::state2string ( State s )
{
#define DOSWITCH(x) case x: return #x
	...
	DOSWITCH ( Paused );
	DOSWITCH ( Stopping );
	case Preparing: return "Allocating/checking files";
	case Searching: return "Connecting to the tracker";
	DOSWITCH ( Connecting );
	DOSWITCH ( WarmingUp );
	DOSWITCH ( Downloading );
	DOSWITCH ( Endgame );
	DOSWITCH ( Seeding );
	}
	return 0;
...
void TorrentClient::setPortRange ( int start,int end )
{
	ServerMinPort = start;
	ServerMaxPort = end;
}

void TorrentClient::setNumUploadSlots ( int num )
{
	MaxUploads = num;
}
