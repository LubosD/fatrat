...
class LogManager : public QObject {
 ...
 public:
  LogManager(QObject* parent, QTextEdit* widgetT, QTextEdit* widgetG)
    ...
    connect(Logger::global(), SIGNAL(logMessage(QString)), widgetG,
  ...
  void setLogSource(Transfer* t) {
    ...
    if (m_last != 0) {
      disconnect(m_last, SIGNAL(logMessage(QString)), m_text,
                 SLOT(append(const QString&)));
      disconnect(m_last, SIGNAL(logMessage(QString)), this,
                 SLOT(onLogMessage()));
    ...
    if (t != 0) {
      ...
      connect(t, SIGNAL(logMessage(QString)), m_text,
              SLOT(append(const QString&)));
