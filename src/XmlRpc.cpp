...
static void insertArgument(QDomDocument& doc, QDomElement& where,
                           const QVariant& what);
static void insertValue(QDomDocument& doc, QDomElement& where, QString type,
                        QString value);
static QVariant parseValue(const QDomElement& where);
static QByteArray createCallOrResponse(const QList<QVariant>& args,
                                       QByteArray* function = 0);

QByteArray XmlRpc::createCall(QByteArray function,
                              const QList<QVariant>& args) {
  return createCallOrResponse(args, &function);
}

QByteArray createCallOrResponse(const QList<QVariant>& args,
                                QByteArray* function) {
  QDomDocument doc;
...
void insertArgument(QDomDocument& doc, QDomElement& where,
  ...
  switch (what.metaType().id()) {
    ...
    case QMetaType::QDateTime:
      insertValue(doc, where, "dateTime.iso8601",
                  what.toDateTime().toString(Qt::ISODate));
      break;
    case QMetaType::QVariantList: {
      ...
      foreach (QVariant v, list) insertArgument(doc, data, v);
    ...
    case QMetaType::QStringList: {
      ...
      foreach (QString v, list) insertArgument(doc, data, v);
    ...
    case QMetaType::QVariantMap: {
      ...
      for (QMap<QString, QVariant>::iterator it = map.begin(); it != map.end();
           it++) {
...
void insertValue(QDomDocument& doc, QDomElement& where, QString type,
                 QString value) {
  QDomElement child = doc.createElement("value");
  QDomElement v;

  if (type.startsWith("ex:"))
    v = doc.createElementNS("http://ws.apache.org/xmlrpc/namespaces/extensions",
                            type);
  else
...
QVariant XmlRpc::parseResponse(const QByteArray& response) {
  ...
  if (!doc.setContent(response, false, &msg)) throw RuntimeException(msg);
  ...
  if (!bFault) {
    ...
    if (value.isNull())
      throw RuntimeException(
          QObject::tr("Server is indicating an unknown failure"));

    QMap<QString, QVariant> v = parseValue(value).toMap();
    throw RuntimeException(QString("%1 %2")
                               .arg(v["faultCode"].toString())
                               .arg(v["faultString"].toString()));
  }
}
...
void XmlRpc::parseCall(const QByteArray& call, QByteArray& function,
                       QList<QVariant>& args) {
  QDomDocument doc;
  QDomElement root;
  QString msg;
  ...
  if (!doc.setContent(call, false, &msg)) throw RuntimeException(msg);
