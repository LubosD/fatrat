...
MainTab::MainTab(QWidget* parent)
    : QTabWidget(parent), m_lastIndex(0), m_lastIndexCur(0) {
...
void MainTab::initAppTools(QMenu* tabOpenMenu) {
  for (int i = 0; i < g_tools.size(); i++) {
    ...
    action =
        tabOpenMenu->addAction(QIcon(":/menu/newtool.png"), g_tools[i].strName);
    action->setData(i);
...
void MainTab::openAppTool() {
  ...
  if (QDialog* dlg = dynamic_cast<QDialog*>(widget)) {
    ...
    connect(widget, SIGNAL(changeTabTitle(QString)), this,
...
void MainTab::closeAllTabs() {
  if (currentIndex() >= FIXED_TAB_COUNT) setCurrentIndex(0);
