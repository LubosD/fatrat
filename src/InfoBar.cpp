...
InfoBar::InfoBar(QWidget* parent, Transfer* d) : QLabel(parent), m_download(d) {
  ...
  connect(d, SIGNAL(destroyed(QObject*)), this,
          SLOT(downloadDestroyed(QObject*)));
...
void InfoBar::refresh() {
  ...
  if (speed.isEmpty()) speed = "- - -";
  if (total) pc = QString("%1").arg(100.0 / total * done, 0, 'f', 1);

  setText(
      QString(
          "<font color=blue>N:</font> %1 | <font color=blue>P:</font> %2% | "
          "<font color=blue>S:</font> %3 | <font color=blue>T:</font> %4")
          .arg(name)
          .arg(pc)
          .arg(speed)
          .arg(time));
...
void InfoBar::removeAll() { qDeleteAll(m_bars); }
