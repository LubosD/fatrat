...
void addSettingsPage(const SettingsItem& i) { g_settingsPages << i; }
...
void setSettingsArray(QString id, QList<QMap<QString, QVariant> >& value) {
  ...
  for (int i = 0; i < value.size(); i++) {
    g_settings->setArrayIndex(i);
    for (QMap<QString, QVariant>::iterator it = value[i].begin();
         it != value[i].end(); it++)
...
void initSettingsDefaults(QString manualPath) {
  ...
  if (!QFile::exists(path)) {
    const char* error =
        "Your installation is incomplete.\nIf you compiled FatRat from source, "
        "please learn how to install programs first!\n";
    std::cerr << error;

    if (getenv("DISPLAY")) QMessageBox::critical(0, "FatRat", error);
    exit(1);
  }
...
void exitSettings() { delete g_settings; }
