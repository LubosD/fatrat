...
Transfer* TransferFactory::createInstance(const char* clsName) {
  if (QThread::currentThread() != thread()) {
    Transfer* t;
    // We need to use Transfer** as Qt doesn't support return values over queued
    // connections
    QMetaObject::invokeMethod(this, "createInstance",
                              Qt::BlockingQueuedConnection,
                              Q_ARG(QString, clsName), Q_ARG(Transfer**, &t));
    return t;
  } else {
...
void TransferFactory::init(Transfer* t, QString source, QString target) {
  ...
  if (QThread::currentThread() != thread()) {
    ...
    QMetaObject::invokeMethod(
        this, "init", Qt::BlockingQueuedConnection, Q_ARG(Transfer*, t),
        Q_ARG(QString, source), Q_ARG(QString, target),
        Q_ARG(RuntimeException*, &e), Q_ARG(bool*, &thrown));
...
void TransferFactory::setState(Transfer* t, Transfer::State state) {
  if (QThread::currentThread() != thread())
    QMetaObject::invokeMethod(this, "setStateSlot", Qt::QueuedConnection,
                              Q_ARG(Transfer*, t),
                              Q_ARG(Transfer::State, state));
  else
    t->setState(state);
}
...
void TransferFactory::init(Transfer* t, QString source, QString target,
                           RuntimeException* pe, bool* eThrown) {
  try {
