...
QModelIndex TorrentFilesModel::index(int row, int column,
...
int TorrentFilesModel::rowCount(const QModelIndex&) const {
  if (!m_download->m_info) return 0;
  return m_download->m_info->files().num_files();
}

QVariant TorrentFilesModel::headerData(int section, Qt::Orientation orientation,
                                       int role) const {
...
QVariant TorrentFilesModel::data(const QModelIndex& index, int role) const {
  ...
  if (!m_download->m_info) return QVariant();

  auto files = m_download->m_info->files();
  if (i >= files.num_files()) return QVariant();
  ...
  if (role == Qt::DisplayRole) {
    switch (index.column()) {
      case 0: {
        ...
        if (p != -1) name = QLatin1String("[...]") + name.mid(p);
      ...
      case 2:
        if (!m_progresses.empty()) {
          int v = int(double(m_progresses[i]) /
                      double(m_download->m_info->files().file_size(i)) * 1000);
...
void TorrentFilesModel::refresh(const libtorrent::bitfield* pieces) {
  ...
  if (m_download->m_info) count = m_download->m_info->files().num_files();

  dataChanged(createIndex(0, 2),
              createIndex(count, m_columns.size()));  // refresh the view
}

void TorrentProgressDelegate::paint(QPainter* painter,
  ...
  if (index.column() == 4 && model->m_pieces && model->m_download) {
    ...
    im = TorrentProgressWidget::generate(libtorrent::bitfield(cpy, end - start),
                                         myrect.width(), buf, sstart, send);
