...
void CurlStat::timeProcess(SpeedData& data, size_t bytes) {
  ...
  if (isNull(data.last))
    ...
    long usec = (tvNow.tv_sec - data.last.tv_sec) * 1000000LL +
                (tvNow.tv_usec - data.last.tv_usec);
    ...
    if (data.max > 0) {
      ...
      if (delta > 0) {
        delta *= 2;
        delta -= 50000;
        if (isNull(data.next)) data.next = tvNow;
...
bool CurlStat::isNull(const timeval& t) { return !t.tv_sec && !t.tv_usec; }
...
bool CurlStat::hasNextReadTime() const { return !isNull(m_down.next); }

bool CurlStat::hasNextWriteTime() const { return !isNull(m_up.next); }

timeval CurlStat::nextReadTime() const { return m_down.next; }

timeval CurlStat::nextWriteTime() const { return m_up.next; }

bool CurlStat::performsLimiting() const { return m_up.max || m_down.max; }

void CurlStat::setMaxUp(int bytespersec) { m_up.max = bytespersec; }

void CurlStat::setMaxDown(int bytespersec) { m_down.max = bytespersec; }

void CurlStat::timeProcessDown(size_t bytes) { timeProcess(m_down, bytes); }

void CurlStat::timeProcessUp(size_t bytes) { timeProcess(m_up, bytes); }
