...
TorrentDownload::~TorrentDownload() {
  ...
  if (m_handle.is_valid()) m_session->remove_torrent(m_handle);
  // delete m_info;
}
...
int TorrentDownload::acceptable(QString uri, bool) {
  ...
  if (istorrent) {
    if (uri[0] == '/' || uri.startsWith("file://")) return 2;
  }
  if (uri.startsWith(MAGNET_PREFIX)) return 3;
  return 0;
}
...
void TorrentDownload::fillSettings(libtorrent::settings_pack& settings) {
  ...
  std::string fingerprint =
      libtorrent::generate_fingerprint("FR", s1, s2, s3, s4);
  ...
  settings.set_str(libtorrent::settings_pack::listen_interfaces,
                   QString("0.0.0.0:%1, [::]:%1").arg(lstart).toStdString());

  settings.set_bool(libtorrent::settings_pack::enable_upnp,
                    getSettingsValue("torrent/mapping_upnp").toBool());
  settings.set_bool(libtorrent::settings_pack::enable_natpmp,
                    getSettingsValue("torrent/mapping_natpmp").toBool());
  settings.set_bool(libtorrent::settings_pack::enable_lsd,
                    getSettingsValue("torrent/mapping_lsd").toBool());
  ...
  settings.set_bool(libtorrent::settings_pack::use_dht_as_fallback,
                    false);  // i.e. use DHT always
  settings.set_int(libtorrent::settings_pack::file_pool_size,
                   getSettingsValue("torrent/maxfiles").toInt());
  settings.set_int(libtorrent::settings_pack::unchoke_slots_limit,
                   getSettingsValue("torrent/maxuploads").toInt());
  settings.set_int(libtorrent::settings_pack::connections_limit,
                   getSettingsValue("torrent/maxconnections").toInt());

  // TODO: Remove cache_size and announce_ip from settings!
  settings.set_int(libtorrent::settings_pack::disk_io_write_mode,
                   getSettingsValue("torrent/disk_io_write_mode").toInt());
  settings.set_int(libtorrent::settings_pack::disk_io_read_mode,
                   getSettingsValue("torrent/disk_io_read_mode").toInt());

  switch (getSettingsValue("torrent/enc_incoming").toInt()) {
    case 0:
      settings.set_int(libtorrent::settings_pack::in_enc_policy,
                       libtorrent::settings_pack::pe_disabled);
      break;
    case 1:
      settings.set_int(libtorrent::settings_pack::in_enc_policy,
                       libtorrent::settings_pack::pe_enabled);
      break;
    case 2:
      settings.set_int(libtorrent::settings_pack::in_enc_policy,
                       libtorrent::settings_pack::pe_forced);
      break;
  }
  switch (getSettingsValue("torrent/enc_outgoing").toInt()) {
    case 0:
      settings.set_int(libtorrent::settings_pack::out_enc_policy,
                       libtorrent::settings_pack::pe_disabled);
      break;
    case 1:
      settings.set_int(libtorrent::settings_pack::out_enc_policy,
  ...
  switch (getSettingsValue("torrent/enc_level").toInt()) {
    case 0:
      settings.set_int(libtorrent::settings_pack::allowed_enc_level,
                       libtorrent::settings_pack::pe_plaintext);
      break;
    case 1:
      settings.set_int(libtorrent::settings_pack::allowed_enc_level,
                       libtorrent::settings_pack::pe_rc4);
      break;
    case 2:
      settings.set_int(libtorrent::settings_pack::allowed_enc_level,
  ...
  settings.set_bool(libtorrent::settings_pack::prefer_rc4,
...
void TorrentDownload::globalInit() {
  if (programHasGUI()) m_labelDHTStats = new QLabel;

  libtorrent::session_params params;

  QByteArray dhtState = getSettingsValue("torrent/dht_state").toByteArray();
  params = libtorrent::read_session_params(
      bdecode_simple(dhtState), libtorrent::session_handle::save_dht_state);
  ...
  if (g_geoIPLib.load()) {
    ...
    GeoIP_country_name_by_addr_imp =
        (const char* (*)(void*, const char*))g_geoIPLib.resolve(
            "GeoIP_country_name_by_addr");
    GeoIP_country_code_by_addr_imp =
        (const char* (*)(void*, const char*))g_geoIPLib.resolve(
  ...
  XmlRpcService::registerFunction(
      "TorrentDownload.setFilePriorities", setFilePriorities,
...
void TorrentDownload::globalExit() {
  if (m_bDHT) {
    libtorrent::entry e;
    auto params =
        m_session->session_state(libtorrent::session_handle::save_dht_state);
    e = libtorrent::write_session_params(
  ...
  if (g_pGeoIP != 0) GeoIP_delete_imp(g_pGeoIP);
}
...
QString TorrentDownload::name() const {
  if (m_handle.is_valid()) {
    ...
    if (st.name.empty() &&
        m_status.state == libtorrent::torrent_status::downloading_metadata)
...
void TorrentDownload::init(QString source, QString target) {
  ...
  try {
    ...
    if (localFile || magnetLink) {
      ...
      storageMode =
          (libtorrent::storage_mode_t)getSettingsValue("torrent/allocation")
              .toInt();
      ...
      if (localFile) {
        ...
        std::shared_ptr<libtorrent::torrent_info> ti =
            std::make_shared<libtorrent::torrent_info>(source.toStdString());
        ...
        if (!isActive()) params.flags |= libtorrent::torrent_flags::paused;

        m_handle = m_session->add_torrent(params);
        // m_handle = m_session->add_torrent(m_info, target.toStdString(),
        // libtorrent::entry(), storageMode, !isActive());
      } else {
        libtorrent::add_torrent_params params =
            libtorrent::parse_magnet_uri(source.toStdString());
      ...
      if (localFile) {
        ...
        if (!m_bAuto) RssFetcher::performManualCheck(name());
      }
    } else {
...
void TorrentDownload::downloadTorrent(QString source) {
  ...
  if (pr.nType != Proxy::ProxyNone) m_pFileDownload->setProxy(pr);
  ...
  connect(m_pFileDownload, SIGNAL(finished(QNetworkReply*)), this,
          SLOT(torrentFileDone(QNetworkReply*)), Qt::QueuedConnection);
...
bool TorrentDownload::storeTorrent(QString orig) {
  ...
  if (!dir.cd(TORRENT_FILE_STORAGE)) return false;
...
bool TorrentDownload::storeTorrent() {
  ...
  if (info) {
    ...
    auto buf = libtorrent::write_torrent_file_buf(
        ap, libtorrent::write_torrent_flags_t(0));
...
void TorrentDownload::torrentFileReadyRead() {
  if (!m_pReply->rawHeader("Location").isEmpty()) return;
...
void TorrentDownload::torrentFileDone(QNetworkReply* reply) {
  ...
  if (reply->error() != QNetworkReply::NoError) {
    ...
    if (!redir.isEmpty()) {
      ...
      connect(m_pReply, SIGNAL(readyRead()), this,
              SLOT(torrentFileReadyRead()));
      return;
    } else {
      try {
...
QString TorrentDownload::object() const { return m_strTarget; }
...
void TorrentDownload::load(const QDomNode& map) {
  ...
  try {
    ...
    std::shared_ptr<libtorrent::torrent_info> ti =
        std::make_shared<libtorrent::torrent_info>(sfile.toStdString());
    m_info = ti;

    torrent_resume =
        QByteArray::fromBase64(getXMLProperty(map, "torrent_resume").toUtf8());

    std::cout << "Loaded " << getXMLProperty(map, "torrent_resume").size()
              << " bytes of resume data\n";

    libtorrent::add_torrent_params params;
    std::vector<char> torrent_resume2 = std::vector<char>(
        torrent_resume.data(), torrent_resume.data() + torrent_resume.size());
    ...
    params.storage_mode =
        libtorrent::storage_mode_sparse;  // don't force full allocation upon
                                          // load
    params.ti = ti;
    ...
    if (!n.isNull()) {
      ...
      std::set_difference(cur_seeds.begin(), cur_seeds.end(), now_seeds.begin(),
                          now_seeds.end(), std::inserter(diff, diff.begin()));
      for (std::set<std::string>::iterator it = diff.begin(); it != diff.end();
           it++)
        m_handle.remove_url_seed(*it);

      diff.clear();

      std::set_difference(now_seeds.begin(), now_seeds.end(), cur_seeds.begin(),
                          cur_seeds.end(), std::inserter(diff, diff.begin()));
      for (std::set<std::string>::iterator it = diff.begin(); it != diff.end();
           it++)
        m_handle.add_url_seed(*it);
    }

    if (isActive()) m_handle.resume();
...
void TorrentDownload::save(QDomDocument& doc, QDomNode& map) const {
    ...
    if (m_handle.is_valid() &&
      ...
      for (int i = 0; i < 3 && !saved; i++) {
        ...
        for (libtorrent::alert* aaa : alerts) {
          if (libtorrent::save_resume_data_alert* alert =
                  libtorrent::alert_cast<libtorrent::save_resume_data_alert>(
                      aaa)) {
            auto entry = libtorrent::write_resume_data_buf(alert->params);
            setXMLProperty(doc, map, "torrent_resume",
                           QByteArray(entry.data(), entry.size()).toBase64());
            saved = true;
          } else if (libtorrent::alert_cast<
                         libtorrent::save_resume_data_failed_alert>(aaa)) {
      ...
      if (!saved) std::cout << "Torrent state did not get saved!\n";
    }
  }
  ...
  setXMLProperty(doc, map, "seedlimitupload",
                 QString::number(m_seedLimitUpload));
  ...
  if (m_handle.is_valid()) {
    ...
    for (size_t i = 0; i < trackers.size(); i++) {
      setXMLProperty(doc, sub, "tracker",
                     QString::fromUtf8(trackers[i].url.c_str()));
    }

    sub = doc.createElement("url_seeds");
    std::set<std::string> seeds = m_handle.url_seeds();
    for (std::set<std::string>::iterator it = seeds.begin(); it != seeds.end();
         it++) {
...
#ifdef WITH_WEBINTERFACE
void TorrentDownload::process(QString method, QMap<QString, QString> args,
                              WriteBack* wb) {
  ...
  if (m_handle.is_valid() && m_info) {
    ...
    if (method == "progress") {
      ...
      if (!pieces.empty())
        TorrentProgressWidget::generate(pieces, WIDTH,
      ...
      QImage img(WIDTH, 1, QImage::Format_RGB32);
      TorrentProgressWidget::generate(avail, WIDTH,
                                      reinterpret_cast<quint32*>(img.bits()));
...
QVariantMap TorrentDownload::properties() const {
  ...
  for (int i = 0; i < m_info->num_files(); i++) {
    ...
    if (m_vecPriorities.size() > i) map["priority"] = int(m_vecPriorities[i]);
    files << map;
  }
  rv["files"] = files;
...
TorrentDownload* TorrentWorker::getByHandle(
    libtorrent::torrent_handle handle) const {
  foreach (TorrentDownload* d, m_objects) {
    if (d->m_handle == handle) return d;
  }
  return 0;
}

void TorrentWorker::processAlert(libtorrent::alert* aaa) {
#define IS_ALERT(type) \
  libtorrent::type* alert = dynamic_cast<libtorrent::type*>(aaa)
#define IS_ALERT_S(type) dynamic_cast<libtorrent::type*>(aaa) != 0
  ...
  if (IS_ALERT(torrent_alert)) {
    ...
    if (IS_ALERT_S(file_error_alert)) {
      ...
      d->enterLogMessage(
          tr("The fast-resume data have been rejected: %1").arg(errmsg));
      ...
      if (!d->m_info) d->m_info = d->m_handle.torrent_file();
      ...
      if (dht_nodes_idx != -1) dht_nodes = alert->counters()[dht_nodes_idx];
      if (dht_torrents_idx != -1)
        dht_torrents = alert->counters()[dht_torrents_idx];

      if (TorrentDownload::m_bDHT && TorrentDownload::m_labelDHTStats &&
          (dht_nodes != -1 || dht_torrents != -1)) {
        QString text = tr("<b>DHT:</b> %1 nodes, %2 torrents tracked")
                           .arg(dht_nodes)
                           .arg(dht_torrents);
...
void TorrentWorker::doWork() {
  ...
  foreach (TorrentDownload* d, m_objects) {
    if (!d->m_handle.is_valid()) continue;

    d->m_status = d->m_handle.status();

    if (!d->m_info) {
      if (!d->m_status.has_metadata) continue;
      d->m_info = d->m_handle.torrent_file();
    }
  }

  foreach (TorrentDownload* d, m_objects) {
    if (!d->m_handle.is_valid() || !d->m_info) continue;

    if (d->m_bHasHashCheck &&
        d->m_status.state != libtorrent::torrent_status::checking_files &&
        d->m_status.state != libtorrent::torrent_status::checking_resume_data) {
      d->m_bHasHashCheck = false;
      // d->m_nPrevDownload = d->done();
    }

    if (d->isActive()) {
      const bool isSuperSeeding =
          d->m_handle.flags() & libtorrent::torrent_flags::super_seeding;
      ...
      if (d->mode() == Transfer::Download) {
        if (d->m_status.state == libtorrent::torrent_status::finished ||
          ...
          d->enterLogMessage(
              tr("Requested parts of the torrent have been downloaded"));
          d->setMode(Transfer::Upload);
        }
      }
      if (d->mode() == Transfer::Upload) {
        if (d->m_status.total_wanted_done < d->m_status.total_wanted)
          ...
          if (double(d->totalUpload()) / d->totalDownload() >=
                  d->m_seedLimitRatio ||
      ...
      if (down != sdown || up != sup) d->setSpeedLimits(sdown, sup);
    }
  }
  ...
  for (libtorrent::alert* aaa : alerts) processAlert(aaa);
}
...
void TorrentDownload::forceRecheck() {
  if (isActive() || !m_handle.is_valid()) return;
...
QVariant TorrentDownload::setFilePriorities(QList<QVariant>& args) {
  ...
  if (!t) throw XmlRpcService::XmlRpcError(102, "Invalid transfer UUID");
