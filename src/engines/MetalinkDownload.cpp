...
MetalinkDownload::MetalinkDownload() : m_network(0), m_file(0), m_reply(0) {}
...
void MetalinkDownload::changeActive(bool nowActive) {
  if (nowActive) {
    if (!m_strSource.startsWith('/')) {
      ...
      QMetaObject::invokeMethod(this, "processMetalink", Qt::QueuedConnection,
                                Q_ARG(QString, m_strSource));
  }
}

void MetalinkDownload::networkReadyRead() { m_file->write(m_reply->readAll()); }
...
void MetalinkDownload::processMetalink(QString fileName) {
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    m_strMessage =
        tr("Failed to read the metalink file: %1").arg(file.errorString());
    setState(Failed);
    return;
  }
  ...
  while (!dfile.isNull()) {
    ...
    while (!snode.isNull()) {
      ...
      } else if (tagName == "metaurl" &&
                 elem.attribute("mediatype") == "torrent") {
  ...
  for (int i = 0; i < files.size(); i++) {
    ...
    if (mode == 0 && !m.hasHTTP) mode = 1;
    if (mode == 1 && !m.hasTorrent) mode = 0;
  ...
  decide:
    ...
    if (mode == 1) {
        ...
        if (m.urls[j].isTorrent) {
          ...
          try {
            ...
            if (!i)
              ...
              this->myQueue()->add(t);
            if (isActive()) t->setState(Waiting);
            this->deleteLater();
      ...
      for (int j = 0; j < m.urls.size(); j++) {
        if (m.urls[j].isTorrent) continue;
...
QString MetalinkDownload::remoteURI() const { return m_strSource; }
