...
TorrentPeersModel::TorrentPeersModel(QObject* parent, TorrentDownload* d)
  ...
  m_columns << tr("Encryption") << tr("Source") << tr("Download")
            << tr("Upload");
  m_columns << tr("Downloaded") << tr("Uploaded") << tr("State");
  m_columns << tr("Completed");
}

QModelIndex TorrentPeersModel::index(int row, int column,
...
QVariant TorrentPeersModel::headerData(int section, Qt::Orientation orientation,
                                       int role) const {
...
QVariant TorrentPeersModel::data(const QModelIndex& index, int role) const {
  if (index.row() >= (int)m_peers.size()) return QVariant();
  ...
  if (role == Qt::DisplayRole) {
    switch (index.column()) {
      ...
      case 9:
        if (info.flags & libtorrent::peer_info::connecting)
          ...
          if (info.flags & libtorrent::peer_info::interesting)
            text += "Interesting ";
          if (info.flags & libtorrent::peer_info::choked) text += "Choked ";
      ...
      case 10: {
        ...
        for (ssize_t i = 0; i < info.pieces.size(); i++)
          if (info.pieces[i]) pcs++;
        QString pct =
            QString("%1%").arg((int)(100.0 / double(info.pieces.size()) * pcs));

        if (info.flags & libtorrent::peer_info::seed) pct += " S";
        return pct;
      }
    }
    ...
    if (index.column() == 1 && g_pGeoIP != 0) {
      std::string ip = info.ip.address().to_string();
      const char* country =
          GeoIP_country_code_by_addr_imp(g_pGeoIP, ip.c_str());
