...
void HttpDetailsBar::paintEvent(QPaintEvent* event) {
  ...
  for (int i = 0; i < m_download->m_segments.size(); i++) {
    ...
    QRect rect = QRect(float(sg.offset) * width / total + 1, 1,
                       float(sg.bytes) * width / total, height - 2);
  ...
  if (ss.client) {
    ...
    text = tr("Segment #%1\nDownload in progress\nURL: %2\nSize: %3\nSpeed: "
              "%4\nDone: %5")
               ...
               .arg(url)
               .arg(size)
               .arg(formatSize(down) + "/s")
               .arg(formatSize(progress));
  } else {
...
int HttpDetailsBar::getSegment(int x) {
  for (int i = 0; i < m_segs.size(); i++) {
    if (x >= m_segs[i].first && x <= m_segs[i].second) return i;
  }
  return -1;
}
...
void HttpDetailsBar::mousePressEvent(QMouseEvent* event) {
  ...
  if (event->button() == Qt::RightButton &&
      m_download != 0 /*&& m_download->isActive()*/) {
    QMenu menu(this);

    m_download->m_segmentsLock.lockForRead();
    if (m_sel >= 0 && m_sel < m_segs.size() &&
        m_download->m_segments[m_sel].client) {
...
void HttpDetailsBar::stopSegment() {
  m_download->m_listActiveSegments.removeOne(
      m_download->m_segments[m_sel].urlIndex);
