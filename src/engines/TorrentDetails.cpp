...
TorrentDetails::TorrentDetails(QWidget* me, TorrentDownload* obj)
  ...
  for (int i = 5; i < 9; i++) hdr->resizeSection(i, 70);
  ...
  connect(treeFiles, SIGNAL(customContextMenuRequested(const QPoint&)), this,
          SLOT(fileContext(const QPoint&)));
  connect(treeFiles, SIGNAL(doubleClicked(const QModelIndex&)), this,
          SLOT(openFile()));
  ...
  connect(treePeers, SIGNAL(customContextMenuRequested(const QPoint&)), this,
          SLOT(peerContext(const QPoint&)));
...
void TorrentDetails::openFile() {
  if (m_selFiles.size() != 1) return;

  int i = m_selFiles[0];

  QString relative =
      QString::fromStdString(m_download->m_info->files().file_path(i));
  QString path = m_download->dataPath(false);

  if (!path.endsWith('/')) path += '/';
  path += relative;

  QProcess::startDetached(getSettingsValue("fileexec").toString(), QStringList()
                                                                       << path);
}
...
void TorrentDetails::fileContext(const QPoint&) {
  if (m_download && m_download->m_handle.is_valid()) {
    ...
    foreach (QModelIndex in, list) {
      ...
      if (row < numFiles) m_selFiles << row;
    }
...
void TorrentDetails::destroy() { m_download = 0; }
...
void TorrentDetails::fill() {
  if (m_download && m_download->m_handle.is_valid() && m_download->m_info) {
    ...
    if (time) {
      std::string created = boost::posix_time::to_simple_string(
          boost::posix_time::from_time_t(time.get()));
      lineCreationDate->setText(created.c_str());
    }
    linePieceLength->setText(
        QString("%1 kB").arg(m_download->m_info->piece_length() / 1024.f));
    ...
    QString magnet = QString::fromStdString(
        libtorrent::make_magnet_uri(m_download->m_handle));
...
void TorrentDetails::refresh() {
  if (m_download && m_download->m_handle.is_valid() && m_download->m_info) {
    ...
    // GENERAL
    int next = std::chrono::duration_cast<std::chrono::seconds>(
                   m_download->m_status.next_announce)
                   .count();
    ...
    if (m_download->m_status.distributed_copies != -1) {
      ...
      lineAvailability->setText(
          QString::number(m_download->m_status.distributed_copies));
    } else
      lineAvailability->setText("-");
    ...
    lineTracker->setText(tr("%1 (refresh in %2:%3:%4)")
                             ...
                             .arg(next / 3600)
                             .arg(next / 60, 2, 10, QChar('0'))
                             .arg(next % 60, 2, 10, QChar('0')));

    libtorrent::bitfield pieces = m_download->m_status.pieces;

    if (pieces.empty() &&
        m_download->m_info->total_size() == m_download->m_status.total_done) {
...
bool TorrentDetails::bitfieldsEqual(const libtorrent::bitfield& b1,
                                    const libtorrent::bitfield& b2) {
  const char *pb1, *pb2;
