...
TorrentPiecesModel::TorrentPiecesModel(QObject* parent, TorrentDownload* d)
  ...
  m_columns << tr("Completed blocks") << tr("Requested blocks")
            << tr("Blocks being written") << tr("Block view");
}

QModelIndex TorrentPiecesModel::index(int row, int column,
...
QVariant TorrentPiecesModel::headerData(int section,
...
QVariant TorrentPiecesModel::data(const QModelIndex& index, int role) const {
  if (role == Qt::DisplayRole) {
    if (index.row() >= (int)m_pieces.size()) return QVariant();
...
void BlockDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
  ...
  if (index.column() == 5)  // block view
  {
    ...
    for (int i = 0; i < piece.blocks_in_piece;) {
      ...
      if (state == libtorrent::block_info::finished)
        painter->fillRect(myrect.x() + from * bwidth, myrect.y(),
                          ceilf(bwidth * (to - from + 1)), myrect.height(),
                          QColor(128, 128, 255));
