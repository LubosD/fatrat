...
class CurlDownload : public StaticTransferMessage<Transfer> {
 ...
 public:
  ...
  static QDialog* createMultipleOptionsWidget(QWidget* parent,
 ...
 private:
  ...
  static size_t process_header(const char* ptr, size_t size, size_t nmemb,
                               CurlDownload* This);
  static int curl_debug_callback(CURL*, curl_infotype, char* text, size_t bytes,
                                 CurlDownload* This);

 protected:
  // Represents a written segment, i.e. what's really been written to the
  // on-disk file
  struct Segment {
    // the start
    qlonglong offset;
    ...
    operator QString() const {
      return QString("(struct Segment): offset: %1; bytes: %2; urlIndex: %3")
          .arg(offset)
          .arg(bytes)
          .arg(urlIndex);
    }
  };
  // Represents a blank spot in the file, i.e. a candidate for a new download
  // thread
  struct FreeSegment {
    FreeSegment(qlonglong _offset, qlonglong _bytes)
        : offset(_offset), bytes(_bytes), affectedClient(0) {}
