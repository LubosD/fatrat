...
void JavaPersistentVariables::setPersistentVariable(QString key, QVariant value)
{
...
{
	if (m_persistent.contains(key))
		return m_persistent[key];
	return QVariant();
}
...
{
	for(QMap<QString,QVariant>::const_iterator it = m_persistent.begin(); it != m_persistent.end(); it++)
		Transfer::setXMLProperty(doc, map, "jpers_"+it.key(), it.value().toString());
}
