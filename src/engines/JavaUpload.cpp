...
void JavaUpload::globalInit() {
  ...
  try {
    ...
    JClass annotation(
        "info.dolezel.fatrat.plugins.annotations.UploadPluginInfo");
...
void JavaUpload::init(QString source, QString target) { setObject(source); }
...
void JavaUpload::changeActive(bool nowActive) {
  ...
  if (nowActive) {
    ...
    // call Java
    m_plugin->call("processFile", JSignature().addString(),
                   JArgs() << m_strSource);
  } else {
...
void JavaUpload::setSpeedLimits(int, int up) { m_up.max = up; }

void JavaUpload::speeds(int& down, int& up) const {
  down = up = 0;

  if (m_curl) CurlUser::speeds(down, up);
}

qulonglong JavaUpload::done() const {
  if (state() == Completed) return m_nTotal;
  if (!isActive() || !m_file.isOpen()) return m_nDone;
  return m_file.pos();
}
...
CURL* JavaUpload::curlHandle() { return m_curl; }
...
size_t JavaUpload::process_header(const char* ptr, size_t size, size_t nmemb,
                                  JavaUpload* This) {
...
void JavaUpload::checkResponse() {
  ...
  m_plugin->call("checkResponse",
                 JSignature().add("java.nio.ByteBuffer").add("java.util.Map"),
                 JArgs() << direct << hdr);
}
...
void JavaUpload::putDownloadLink(QString downloadLink, QString killLink) {
  ...
  if (!killLink.isEmpty()) enterLogMessage(tr("Kill link: %1").arg(killLink));
  setState(Completed);
}

void JavaUpload::startUpload(QString url, QList<JavaUpload::MimePart>& parts,
                             qint64 offset, qint64 bytes) {
  ...
  foreach (const MimePart& part, parts) {
    ...
    if (!part.filePart) {
      ...
      curl_formadd(&m_postData, &lastData, CURLFORM_COPYNAME,
                   fieldName.constData(), CURLFORM_COPYCONTENTS,
                   value.constData(), CURLFORM_END);
    } else {
      ...
      curl_formadd(&m_postData, &lastData, CURLFORM_COPYNAME,
                   fieldName.constData(), CURLFORM_STREAM,
                   static_cast<CurlUser*>(this), CURLFORM_FILENAME, m_fileName,
