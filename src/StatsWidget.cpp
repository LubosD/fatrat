...
void StatsWidget::refresh() {
  ...
  if (!m_strInterface.isEmpty()) newv = getInterfaceStats(m_strInterface);
...
void StatsWidget::paintEvent(QPaintEvent* event) {
  ...
  if (event != 0) painter.setClipRegion(event->region());
  ...
  if (m_globDownPrev >= 0 && m_globDown >= 0 && height() > SPACE * 2) {
    ...
    painter.fillRect(width() - colwidth, SPACE, -colwidth, colheight,
                     Qt::white);
    painter.drawRect(width() - colwidth, SPACE, -colwidth, colheight);

    tmp = double(colheight) / maxup * globup;
    painter.fillRect(width() - colwidth, colheight + SPACE, -colwidth, -tmp,
                     Qt::black);
    ...
    painter.drawText(QRect(width() - colwidth * 3, 0, colwidth * 3, SPACE),
                     Qt::AlignCenter, "up");
    painter.drawText(QRect(0, height() - SPACE, width(), SPACE),
                     Qt::AlignCenter, m_strInterface);
  }
}
