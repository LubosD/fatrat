...
void initTransferClasses() {
  ...
  {  // couldn't look more lazy and lame
    EngineEntry e = {"FakeDownload",
                     "Fake engine",
                     0,
                     0,
                     {FakeDownload::createInstance},
                     {FakeDownload::acceptable},
                     0,
                     0};
    g_enginesDownload << e;
  }
...
#ifdef WITH_BITTORRENT
  {
    EngineEntry e = {"TorrentDownload",
                     "BitTorrent download",
                     TorrentDownload::globalInit,
                     TorrentDownload::globalExit,
                     {TorrentDownload::createInstance},
                     {TorrentDownload::acceptable},
                     0};
    g_enginesDownload << e;
  }
#endif
#ifdef WITH_CURL
  {
    EngineEntry e = {"GeneralDownload",
                     "CURL HTTP(S)/FTP(S)/SFTP download",
                     CurlDownload::globalInit,
                     CurlDownload::globalExit,
                     {CurlDownload::createInstance},
                     {CurlDownload::acceptable},
                     CurlDownload::createMultipleOptionsWidget};
    g_enginesDownload << e;
  }
  {
    EngineEntry e = {"FtpUpload",
                     "CURL FTP(S)/SFTP upload",
                     0,
                     0,
                     {CurlUpload::createInstance},
                     {CurlUpload::acceptable},
                     0};
    g_enginesUpload << e;
  }
  {
    EngineEntry e = {"MetalinkDownload",
                     "Metalink file handler",
                     MetalinkDownload::globalInit,
                     0,
                     {MetalinkDownload::createInstance},
                     {MetalinkDownload::acceptable},
                     0};
    g_enginesDownload << e;
  }
#endif
...
Transfer::State Transfer::state() const { return m_state; }
...
void Transfer::setInternalSpeedLimits(int down, int up) {
  if ((m_nDownLimit < down && m_nDownLimit) || !down) down = m_nDownLimit;
  if ((m_nUpLimit < up && m_nUpLimit) || !up) up = m_nUpLimit;
...
int Transfer::getEngineID(QString className, Mode type) {
  if (type == Download) {
    for (int i = 0; i < g_enginesDownload.size(); i++) {
      if (className == g_enginesDownload[i].shortName) return i;
    }
  } else if (type == Upload) {
    for (int i = 0; i < g_enginesUpload.size(); i++) {
      if (className == g_enginesUpload[i].shortName) return i;
    }
  }
...
bool Transfer::runProperties(QWidget* parent, Mode mode, int classID,
                             QList<Transfer*> objects) {
  const EngineEntry* entries = engines(mode);
  bool err = false;

  if (!entries[classID].lpfnMultiOptions) err = true;
  if (!err) {
    int result;
  ...
  if (err)
    QMessageBox::information(
        parent, "FatRat",
        tr("This transfer type has no advanced options to set."));
  return true;
}

const EngineEntry* Transfer::engines(Mode type) {
  return (type == Download) ? g_enginesDownload.constData()
                            : g_enginesUpload.constData();
}
...
Transfer::BestEngine Transfer::bestEngine(QString uri, Mode type) {
  ...
  if (type != Upload) {
    for (int i = 0; i < g_enginesDownload.size(); i++) {
      ...
      if (!g_enginesDownload[i].lpfnAcceptable) continue;

      n = g_enginesDownload[i].lpfnAcceptable2(uri, type == ModeInvalid,
                                               &g_enginesDownload[i]);
  ...
  if (type != Download) {
    for (int i = 0; i < g_enginesUpload.size(); i++) {
      ...
      if (!g_enginesUpload[i].lpfnAcceptable) continue;

      n = g_enginesUpload[i].lpfnAcceptable2(uri, type == ModeInvalid,
                                             &g_enginesUpload[i]);
...
void Transfer::setState(State newState) {
  ...
  if (newState == m_lastState) return;

  enterLogMessage(tr("Changed state: %1 -> %2")
                      .arg(state2string(m_state))
                      .arg(state2string(newState)));

  if (newState == Completed) fireCompleted();
  ...
  if (!m_bLocal)
    emit TransferNotifier::instance()->stateChanged(this, m_lastState,
                                                    newState);
...
void Transfer::setXMLProperty(QDomDocument& doc, QDomNode& node, QString name,
                              QString value) {
...
void Transfer::fireCompleted() {
  if (m_strCommandCompleted.isEmpty()) return;
...
QString Transfer::autoActionCommand(State state) const {
  if (state == Completed) return m_strCommandCompleted;
  return QString();
}

void Transfer::setAutoActionCommand(State state, QString command) {
  if (state == Completed) m_strCommandCompleted = command;
}

QString Transfer::stateString() const { return state2string(state()); }

void Transfer::setStateString(QString s) { setState(string2state(s)); }

void Transfer::setSpeedLimits(int down, int up) {}

QString Transfer::uuid() const { return m_uuid.toString(); }
...
Queue* Transfer::myQueue() const {
  QReadLocker l(&g_queuesLock);
  foreach (Queue* q, g_queues) {
    if (q->contains(const_cast<Transfer*>(this))) return q;
  }
  return 0;
}
...
TransferNotifier* TransferNotifier::instance() { return transferNotifier(); }
