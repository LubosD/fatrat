...
void JPlugin::abort() {
  for (QMap<QNetworkReply*, RequesterReceiver>::iterator it =
           m_fetchCallbacks.begin();
       it != m_fetchCallbacks.end();) {
...
void JPlugin::registerNatives() {
  ...
  natives << JNativeMethod(
      ...
      JSignature()
          .addString()
          .add("info.dolezel.fatrat.plugins.listeners.PageFetchListener")
          .addString()
          .add("java.util.Map"),
      fetchPage);
...
void JPlugin::fetchPage(JNIEnv* env, jobject jthis, jstring jurl,
                        jobject cbInterface, jstring postData, jobject jmap) {
  JPlugin* This = getCObject(jthis);
  QString url = JString(jurl).toString();
  QNetworkReply* reply;

  if (This->m_transfer)
    This->m_transfer->enterLogMessage(QLatin1String("JPlugin::fetchPage(): ") +
                                      url);

  qDebug() << "JPlugin::fetchPage():" << url;
  if (postData) qDebug() << "postData:" << JString(postData).str();
  ...
  if (jmap) {
    ...
    for (QMap<QString, QString>::iterator it = qmap.begin(); it != qmap.end();
         it++)
...
void JPlugin::fetchFinished(QNetworkReply* reply) {
  ...
  try {
    if (reply->error() != QNetworkReply::NoError) {
      if (m_transfer)
        m_transfer->logMessage(QLatin1String("JPlugin::fetchFinished(): ") +
                               reply->errorString());
...
bool JPlugin::checkIfAlive() {
  if (!m_fetchCallbacks.isEmpty()) return true;
  return m_bTaskDone;
}
