...
void JBackgroundWorker::registerNatives() {
  ...
  JClass("info.dolezel.fatrat.plugins.util.BackgroundWorker")
...
jobject JBackgroundWorker::get(JNIEnv*, jobject jthis) {
  ...
  if (This->m_exception.isNull())
    ...
    JObject ex("java.util.concurrent.ExecutionException",
               JSignature().add("java.lang.Throwable"),
               JArgs() << This->m_exception);
...
void JBackgroundWorker::updateProgress(JNIEnv*, jobject jthis, jobject p) {
  QMetaObject::invokeMethod(getCObjectAuto(jthis), "progressUpdated",
                            Q_ARG(JObject, JObject(p)));
}

void JBackgroundWorker::progressUpdated(JObject p) {
  call("progressUpdated", JSignature().add("java.lang.Object"), JArgs() << p);
}
void JBackgroundWorker::finished() { call("done"); }

void JBackgroundWorker::run() {
  try {
    m_result = call("doInBackground", JSignature().ret("java.lang.Object"))
                   .value<JObject>();
