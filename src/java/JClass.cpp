...
JClass::JClass(QString clsName) {
  ...
  if (!obj)
    throw RuntimeException(
        QObject::tr("Java class %1 not found or failed to load").arg(clsName));
...
JClass::operator jclass() const { return m_class; }
...
QVariant JClass::callStatic(const char* name, const char* sig,
                            QList<QVariant> args) {
  JScope s;
  ...
  switch (*rvtype) {
    ...
    case 'L': {
      ...
      jobject obj = env->CallStaticObjectMethodA(m_class, mid, jargs);
      if (obj && env->IsInstanceOf(obj, string_class) &&
          !strcmp(rvtype + 1, "java/lang/String;"))
...
QString JClass::getClassName() const {
  ...
  JObject obj(m_class);
  QList<QVariant> args;
  return obj.call("getCanonicalName", JSignature().retString(), args)
      .toString();
}
...
void JClass::registerNativeMethods(const QList<JNativeMethod>& m) {
  ...
  for (int i = 0; i < m.size(); i++) nm[i] = m[i].toStruct();
