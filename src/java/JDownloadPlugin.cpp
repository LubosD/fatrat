...
QMap<QString, JDownloadPlugin::RequesterReceiver>
    JDownloadPlugin::m_captchaCallbacks;
...
JDownloadPlugin::JDownloadPlugin(const char* clsName, const char* sig,
                                 JArgs args)
...
void JDownloadPlugin::registerNatives() {
  ...
  natives << JNativeMethod(
      "startDownload",
      JSignature().add("info.dolezel.fatrat.plugins.extra.DownloadUrl"),
      startDownload);
  natives << JNativeMethod(
      "startWait",
      JSignature().addInt().add(
          "info.dolezel.fatrat.plugins.listeners.WaitListener"),
      startWait);
  natives << JNativeMethod(
      "solveCaptcha",
      JSignature().addString().add(
          "info.dolezel.fatrat.plugins.listeners.CaptchaListener"),
      solveCaptcha);
  natives << JNativeMethod("reportFileName", JSignature().addString(),
                           reportFileName);

  JClass("info.dolezel.fatrat.plugins.DownloadPlugin")
...
void JDownloadPlugin::captchaSolved(QString url, QString solution) {
  ...
  JDownloadPlugin* This =
      static_cast<JDownloadPlugin*>(m_captchaCallbacks[url].first);

  // Needs to be called from the right thread
  QMetaObject::invokeMethod(This, "captchaSolvedSlot", Qt::QueuedConnection,
                            Q_ARG(QString, url), Q_ARG(QString, solution));
}
...
void JDownloadPlugin::captchaSolvedSlot(QString url, QString solution) {
  ...
  try {
    if (!solution.isEmpty()) {
      m_transfer->enterLogMessage(
          QLatin1String("JDownloadPlugin::captchaSolved(): ") + url + " -> " +
          solution);
...
void JDownloadPlugin::solveCaptcha(JNIEnv* env, jobject jthis, jstring jurl,
                                   jobject cb) {
  JDownloadPlugin* This = static_cast<JDownloadPlugin*>(getCObject(jthis));
  QString url = JString(jurl);

  This->m_transfer->enterLogMessage(
      QLatin1String("JDownloadPlugin::solveCaptcha(): ") + url);
  m_captchaCallbacks[url] = RequesterReceiver(getCObject(jthis), cb);

  Captcha::processCaptcha(url, captchaSolved);
}

void JDownloadPlugin::reportFileName(JNIEnv* env, jobject jthis,
                                     jstring jname) {
  JDownloadPlugin* This = static_cast<JDownloadPlugin*>(getCObject(jthis));
  JString name(jname);
  ...
  This->m_transfer->enterLogMessage(
      QLatin1String("JDownloadPlugin::reportFileName(): ") + name.str());
}

void JDownloadPlugin::startDownload(JNIEnv*, jobject jthis,
                                    jobject jdownloadUrl) {
  JDownloadPlugin* This = static_cast<JDownloadPlugin*>(getCObject(jthis));
  JObject downloadUrl(jdownloadUrl);

  QString url = downloadUrl.getValue("url", JSignature::sigString()).toString();
  QString ref =
      downloadUrl.getValue("referrer", JSignature::sigString()).toString();
  QString ua =
      downloadUrl.getValue("userAgent", JSignature::sigString()).toString();
  QString fileName =
      downloadUrl.getValue("fileName", JSignature::sigString()).toString();
  QString postData =
      downloadUrl.getValue("postData", JSignature::sigString()).toString();
  ...
  if (!fileName.isEmpty()) url += "#__filename=" + fileName.replace('/', '-');

  static_cast<JavaDownload*>(This->m_transfer)
      ->startDownload(url, c, ref, ua, postData);

  This->m_network->setCookieJar(new QNetworkCookieJar);
  This->m_bTaskDone = true;
}

QMap<QString, QString> JDownloadPlugin::cookiesToMap(
    const QList<QNetworkCookie>& list) {
  QMap<QString, QString> rv;
  foreach (QNetworkCookie c, list) rv[c.name()] = c.value();
  return rv;
}

void JDownloadPlugin::startWait(JNIEnv* env, jobject jthis, jint seconds,
                                jobject cbInterface) {
  JDownloadPlugin* This = static_cast<JDownloadPlugin*>(getCObject(jthis));
  This->m_transfer->enterLogMessage(
      QString("JDownloadPlugin::startWait(): %1").arg(seconds));
...
void JDownloadPlugin::abort() {
  ...
  for (QMap<QString, RequesterReceiver>::iterator it =
           m_captchaCallbacks.begin();
       it != m_captchaCallbacks.end();) {
...
bool JDownloadPlugin::checkIfAlive() {
  if (!m_waitCallback.isNull()) return true;

  for (QMap<QString, RequesterReceiver>::iterator it =
           m_captchaCallbacks.begin();
       it != m_captchaCallbacks.end(); it++) {
    if (it.value().first == this) return true;
  }
