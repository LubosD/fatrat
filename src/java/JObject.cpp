...
JObject::JObject() : m_bWeak(false) { m_object = 0; }

JObject::JObject(const JObject& obj) : m_bWeak(obj.m_bWeak) {
...
JObject::JObject(jobject obj, bool weak) : m_bWeak(weak) {
  JNIEnv* env = *JVM::instance();
  m_object = obj;
...
void JObject::construct(const JClass& xcls, const char* sig, JArgs args) {
  ...
  if (!cls) throw RuntimeException(QObject::tr("Invalid JClass passed"));
  ...
  if (!ex.isNull()) {
    ...
    QString message =
        ex.call("getMessage", JSignature().retString()).toString();
  ...
  if (!obj)
    throw RuntimeException(
        QObject::tr("Failed to create an instance - constructor %1").arg(sig));
...
JObject::JObject(const JClass& xcls, JSignature sig, JArgs args) : m_object(0) {
...
JObject::operator jobject() { return m_object; }

JObject::~JObject() { setNull(); }

bool JObject::isString() const { return instanceOf("java/lang/String"); }

JString JObject::toStringShallow() const {
  if (!isString()) return JString();
  return JString(jstring(m_object));
}

QString JObject::toString() const {
  if (!m_object) return JString();
  return const_cast<JObject*>(this)
      ->call("toString", JSignature().retString())
      .toString();
}
...
QVariant JObject::call(const char* name, const char* sig, JArgs args) {
  ...
  if (!mid)
    throw RuntimeException(
        QObject::tr("Method %1 %2 not found").arg(name).arg(sig));
  ...
  if (!rvtype)
    throw RuntimeException(
        QObject::tr("Invalid method return type").arg(name).arg(sig));
  rvtype++;
  ...
  switch (*rvtype) {
    ...
    default:
      throw RuntimeException(
          QObject::tr("Unknown Java data type: %1").arg(rvtype));
  }
...
QVariant JObject::getValue(const char* name, const char* sig) const {
  ...
  if (!fid)
    throw RuntimeException(
        QObject::tr("Field %1 %2 not found").arg(name).arg(sig));
  ...
  switch (sig[0]) {
    ...
    default:
      throw RuntimeException(
          QObject::tr("Unknown Java data type: %1").arg(sig[0]));
  }
}
