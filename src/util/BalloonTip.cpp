...
BalloonTip::BalloonTip(QWidget *parent, QIcon si, const QString &title,
...
void BalloonTip::balloon(const QPoint &pos, int msecs, bool showArrow) {
  ...
  setContentsMargins(border + 3, border + (arrowAtTop ? ah : 0) + 2, border + 3,
  ...
  if (msecs > 0) m_timer.start(msecs);
  show();
}

void BalloonTip::mousePressEvent(QMouseEvent *e) {
  close();
  if (e->button() == Qt::LeftButton) emit messageClicked();
}
