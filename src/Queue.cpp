...
Queue::Queue()
    : m_nDownLimit(0),
      m_nUpLimit(0),
      m_nDownTransferLimit(1),
      m_nUpTransferLimit(1),
      m_nDownAuto(0),
      m_nUpAuto(0),
      m_bUpAsDown(false),
...
void Queue::stopQueues() {
  ...
  for (int i = 0; i < g_queues.size(); i++) {
    ...
    for (int j = 0; j < q->size(); j++) {
      Transfer* t = q->at(j);
      if (t->isActive()) t->changeActive(false);
    }
    q->unlock();
  }
}
...
void Queue::loadQueues() {
  ...
  if (!dir.cd(".local/share/fatrat")) return;
  file.setFileName(dir.absoluteFilePath("queues.xml"));

  QString errmsg;
  if (!file.open(QIODevice::ReadOnly) ||
      !doc.setContent(&file, false, &errmsg)) {
    qDebug() << "Failed to open " << file.fileName();
    if (!errmsg.isEmpty()) qDebug() << "PARSE ERROR!" << errmsg;
    ...
    while (!n.isNull()) {
      if (!n.hasAttribute("name"))
        ...
        pQueue->m_strDefaultDirectory =
            n.attribute("defaultdir", pQueue->m_strDefaultDirectory);
...
void Queue::BackgroundSaver::run() {
  Queue::saveQueues();
  if (getSettingsValue("queue_synconwrite").toBool()) sync();
}
...
void Queue::saveQueues() {
  ...
  if (file.write(doc.toByteArray()) == -1 || !file.flush())
    Logger::global()->enterLogMessage(tr("Queue"),
                                      tr("Failed to write the queue file!"));
  else {
    file.close();
  ...
  while (!n.isNull()) {
    ...
    if (d != 0) {
      ...
      */
      ...
      qDebug() << "***ERROR*** Unable to createInstance "
               << n.attribute("class");
...
Transfer* Queue::take(int n, bool nolock) {
  ...
  if (!nolock) m_lock.lockForWrite();
  if (n < size() && n >= 0) d = m_transfers.takeAt(n);
  if (!nolock) m_lock.unlock();
...
void Queue::remove(int n, bool nolock) {
  ...
  if (d->isActive()) d->setState(Transfer::Paused);
  d->deleteLater();
}
...
bool Queue::replace(Transfer* old, QList<Transfer*> _new) {
  ...
  for (int j = 0; j < _new.size(); j++) m_transfers.insert(i + j, _new[j]);
  return true;
}
...
void Queue::stopAll() {
  ...
  for (int j = 0; j < size(); j++) {
    Transfer* t = at(j);
    if (t->isActive()) t->setState(Transfer::Paused);
  }
}
