...
void CreateTorrentDlg::browseFiles() {
  ...
  if (!r.isEmpty()) lineData->setText(r);
}

void CreateTorrentDlg::browseDirs() {
  QString r =
      QFileDialog::getExistingDirectory(this, "FatRat", lineData->text());
  if (!r.isEmpty()) lineData->setText(r);
}
...
void CreateTorrentDlg::createTorrent() {
  ...
  info = new libtorrent::create_torrent(
      m_fs, 64 * 1024 * pow(2, comboPieceSize->currentIndex()));
  ...
  for (int i = 0; i < listTrackers->count(); i++) {
    ...
    if ((text.startsWith("http://") || text.startsWith("udp://")) &&
        text.size() > 7)
...
void CreateTorrentDlg::recurseDir(QList<QPair<QString, qint64> >& list,
                                  QString prefix, QString path) {
  QDir dir(path);
  QFileInfoList flist = dir.entryInfoList();

  for (int i = 0; i < flist.size(); i++) {
    if (flist[i].fileName() == "." || flist[i].fileName() == "..") continue;

    if (flist[i].isDir())
      recurseDir(list, prefix + flist[i].fileName() + '/',
                 flist[i].absoluteFilePath());
    else
      list << QPair<QString, qint64>(prefix + flist[i].fileName(),
                                     flist[i].size());
  }
}
...
void CreateTorrentDlg::hasherFinished() {
  ...
  if (m_hasher->error().isEmpty()) {
    torrent = QFileDialog::getSaveFileName(this, "FatRat", QString(),
                                           tr("Torrents (*.torrent)"))
                  .toUtf8();
    if (!torrent.isEmpty()) {
...
HasherThread::HasherThread(QByteArray baseDir, libtorrent::create_torrent* info,
                           QObject* parent)
...
void HasherThread::run() {
  try {
    libtorrent::set_piece_hashes(*m_info, m_baseDir.data(), [&](int p) {
      if (m_bAbort) throw abort_exception();
