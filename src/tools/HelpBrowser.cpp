...
HelpBrowser::HelpBrowser() {
  ...
  if (!m_engine->setupData()) {
    QMessageBox::critical(this, tr("Help"),
                          tr("Failed to load the documentation."));
    deleteLater();
    return;
  }
  ...
  connect(content, SIGNAL(linkActivated(const QUrl&)), this,
          SLOT(openPage(const QUrl&)));
  connect(content, SIGNAL(clicked(const QModelIndex&)), this,
          SLOT(itemClicked(const QModelIndex&)));
  connect(textBrowser, SIGNAL(sourceChanged(const QUrl&)), this,
          SLOT(sourceChanged()));
...
void HelpBrowser::openPage(const QUrl& url) { textBrowser->setSource(url); }
