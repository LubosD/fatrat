...
int main(int argc, char** argv) {
  ...
  if (!m_strPidFile.isEmpty()) writePidFile();
  if (!m_strSetUser.isEmpty()) dropPrivileges();
  ...
  if (!m_bForceNewInstance) processSession(arg);
  ...
  if (m_bStartGUI) initSettingsPages();
  ...
  if (!arg.isEmpty() && m_bStartGUI) g_wndMain->addTransfer(arg);

  new Scheduler;

  if (m_bStartGUI) QApplication::setQuitOnLastWindowClosed(false);
  rval = app->exec();
...
void processSession(QString arg) {
  ...
  if (bus->isServiceRegistered("info.dolezel.fatrat")) {
    ...
    if (!arg.isEmpty()) {
      ...
      QDBusInterface iface("info.dolezel.fatrat", "/", "info.dolezel.fatrat",
                           conn);

      iface.call("addTransfers", arg);
    } else {
      QMessageBox::critical(0, "FatRat",
                            QObject::tr("There is already a running instance.\n"
                                        "If you want to start FatRat anyway, "
                                        "pass --force among arguments."));
    }
...
static void runEngines(bool init) {
  for (int i = 0; i < g_enginesDownload.size(); i++) {
    if (init) {
      if (g_enginesDownload[i].lpfnInit) g_enginesDownload[i].lpfnInit();
    } else {
      if (g_enginesDownload[i].lpfnExit) g_enginesDownload[i].lpfnExit();
    }
  }

  for (int i = 0; i < g_enginesUpload.size(); i++) {
    if (init) {
      if (g_enginesUpload[i].lpfnInit) g_enginesUpload[i].lpfnInit();
    } else {
      if (g_enginesUpload[i].lpfnExit) g_enginesUpload[i].lpfnExit();
    }
  }
}

QWidget* getMainWindow() { return g_wndMain; }
...
QString formatSize(qulonglong size, bool persec) {
  ...
  if (size < 1024)
    ...
    rval = QString("%L1 GB").arg(double(size) / 1024.0 / 1024.0 / 1024.0, 0,
                                 'f', 1);
...
QString formatTime(qulonglong inval) {
  ...
  if (days) result = QString("%1d ").arg(days);
  if (hrs) result += QString("%1h ").arg(hrs);
  if (mins) result += QString("%1m ").arg(mins);
  if (secs) result += QString("%1s").arg(secs);
...
class RecursiveRemove : public QThread {
 public:
  RecursiveRemove(QString what) : m_what(what) { start(); }
  void run() {
  ...
  static bool work(QString what) {
    ...
    if (!QFile::exists(what)) return true;  // silently ignore
    if (!QFile::remove(what)) {
      ...
      foreach (QString item, contents) {
        if (item != "." && item != "..") {
          if (!work(dir.filePath(item))) return false;
        }
      }
...
void recursiveRemove(QString what) { new RecursiveRemove(what); }

bool openDataFile(QFile* file, QString filePath) {
  if (filePath[0] != '/') filePath.prepend('/');

  file->setFileName(QDir::homePath() + QLatin1String(USER_PROFILE_PATH) +
                    filePath);
  ...
  if (!file->open(QIODevice::ReadOnly)) {
    Logger::global()->enterLogMessage(
        QObject::tr("Unable to load a data file:") + ' ' + filePath);
    return false;
  } else
    return true;
}
...
QString getDataFileDir(QString dir, QString fileName) {
  if (fileName.isEmpty()) return QLatin1String(DATA_LOCATION) + dir;

  QString f = QDir::homePath() + QLatin1String(USER_PROFILE_PATH) + dir;
  if (fileName[0] != '/') fileName.prepend('/');
...
QStringList listDataDir(QString path) {
  ...
  foreach (QString str, entries) rv[str] = dir.filePath(str);
...
void loadPlugins() {
  ...
  if (const char* p = getenv("PLUGIN_PATH")) loadPlugins(p);
}
...
void loadPlugins(const char* p) {
  ...
  foreach (QString pl, plugins) {
    ...
    if (void* l = dlopen(p.constData(), RTLD_NOW)) {
      Logger::global()->enterLogMessage(QObject::tr("Loaded a plugin:") + ' ' +
                                        pl);
      ...
      if (info != 0) {
        ...
        if (strcmp(i.version, VERSION)) {
          qDebug() << "WARNING! Version conflict." << pl << "is" << i.version
                   << "but FatRat is" << VERSION;
          Logger::global()->enterLogMessage(
              QObject::tr("WARNING: the plugin is incompatible:") + ' ' + pl);
        }
      }
    } else
      Logger::global()->enterLogMessage(
          QObject::tr("Failed to load a plugin: %1: %2")
              .arg(pl)
              .arg(dlerror()));
  }
}

bool programHasGUI() { return m_bStartGUI; }

void showHelp() {
  std::cout
      << "FatRat download manager (" VERSION
         ")\n\n"
         "Copyright (C) 2006-2011 Lubos Dolezel\n"
         "Licensed under the terms of the GNU GPL version 3 as published by "
         "the Free Software Foundation\n\n"
         "-f, --force      \tRun the program even if an instance already "
         "exists\n"
         "-i, --hidden     \tHide the GUI at startup (only if the tray icon "
         "exists)\n"
  ...
  //"--force-jre-search\tIgnore the cached JRE location\n"
         ...
         "If started in the GUI mode, you may pass transfers as arguments and "
         "they will be presented to the user\n";
  exit(0);
}
...
void __attribute__((unused)) testJava() {
  try {
    ...
    JObject obj =
        system.getStaticValue("out", JSignature::sig("java.io.PrintStream"))
            .value<JObject>();
...
void daemonize() { daemon(true, false); }
...
void testNotif() {
  ...
  if (iface->isValid()) {
    QDBusPendingReply<uint> reply = iface->Notify(
        "fatrat", 0, "", "Summary", "Body", QStringList(), QVariantMap(), -1);
    reply.waitForFinished();
  }

  OrgKdeKNotifyInterface* iface2 =
      new OrgKdeKNotifyInterface(OrgKdeKNotifyInterface::staticInterfaceName(),
                                 "/Notify", QDBusConnection::sessionBus());
  if (iface2->isValid()) {
    iface2->event("warning", "fatrat", QVariantList(), "Summary", "Body",
                  QByteArray(), QStringList(), 5000, 0);
  }
}
...
void dropPrivileges() {
  ...
  if (!group.isEmpty()) {
    ...
    if (!g) {
      Logger::global()->enterLogMessage(
          "Cannot getgrnam() - invalid group name?");
