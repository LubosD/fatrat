...
TransfersModel::~TransfersModel() { qDeleteAll(m_states, m_states + 12); }

QModelIndex TransfersModel::index(int row, int column,
...
QVariant TransfersModel::headerData(int section, Qt::Orientation orientation,
                                    int role) const {
...
TransfersModel::RowData TransfersModel::createDataSet(Transfer* t) {
  ...
  newData.progress =
      (total) ? QString("%1%").arg(newData.fProgress, 0, 'f', 1) : QString();
  ...
  if (t->isActive()) {
    ...
    if (t->total()) {
      ...
      if (t->primaryMode() == Transfer::Download) {
        if (down) newData.timeLeft = formatTime(totransfer / down);
      } else if (up)
...
void TransfersModel::refresh() {
  ...
  if (q != 0) {
    ...
    for (int i = 0, j = 0; i < count; i++, j++) {
      ...
      if (!filter.isEmpty()) m_filterMapping[j] = i;
      if (!filter.isEmpty() &&
          !t->name().contains(filter, Qt::CaseInsensitive)) {
        filtered++;
        j--;
        continue;
      }

      if (t != 0) newData = createDataSet(t);
  ...
  for (int i = 0; i < count;) {
    if (!changes[i])
      ...
      int from = i, to;
      while (i < count && changes[i]) to = i++;

      dataChanged(createIndex(from, 0),
                  createIndex(to, 5));  // refresh the view
    }
  }
}
...
int TransfersModel::columnCount(const QModelIndex&) const { return 7; }
    ...
    if (index.column() == 0 && row < m_lastData.size()) {
      ...
      if (m_lastData[row].mode == Transfer::Upload) {
        if (state == Transfer::Completed &&
            m_lastData[row].primaryMode == Transfer::Download)
  ...
  if (index.column() == 1) {
    ...
    if (row < model->m_lastData.size()) {
      if (model->m_lastData[row].size != "?")
        opts.text =
            QString("%1%").arg(model->m_lastData[row].fProgress, 0, 'f', 1);
      else
        opts.text = "?";
